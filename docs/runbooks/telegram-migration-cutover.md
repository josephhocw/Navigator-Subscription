...
> - [x] The `functions` maxDuration entries took effect — verified empirically 2026-08-17: the 13 Aug noon sweep ran 4m21s in one invocation (Status Log rows 12:29:55→12:34:16), impossible under a 60s cap, so the 300s setting is live; the webhook completes joins in seconds.
...
> **✅ PHASE C CONFIGURED 2026-08-07 — soak day 1.** Shadow bot @RhoNavStaginBot (`8770545037`) is admin in the 5 real groups (Joseph, 2026-08-07). Staging env repointed via CLI (real chat IDs, live sheet ID, `TELEGRAM_JOIN/SWEEP/KICK_DRY_RUN=true`) + redeployed; webhook registration unchanged (same URL + secret) so no `setWebhook` re-run was needed — endpoint verified live (403 without the secret header). Prod: five `TELEGRAM_CHAT_*` + `TELEGRAM_KICK_WHITELIST` set + redeployed (see prod-env note above); flags unset = dry-run, so the prod sweep pings intentions daily at ~noon SGT — that ping is stream (b). **`RhoNavStaginBot` is appended to every whitelist** (bot repo `config.py` commit `3229a33`, prod env, staging env) so no code path can kick the shadow bot. VPS bots pulled + restarted 2026-08-07 (carry `TRIAL_CANCELLED` barring + the new whitelist entry). Daily from here: run the step-4 diffs and log divergences below.
...
- **2026-08-13 — sweep drift cleared by hand (Joseph-approved).** The dry-run sweep's flagged plan-drift cases were recomputed from the live sheet + live membership (139 candidates, same logic) and executed manually via ban+unban with the prod token: @yk_cyk_yk (US, was ALL_MARKETS) out of HK/SG/FXMC; @Josephlim2709 (US) out of HK/SG/FXMC; @starzcrab (US) out of HK; @Linnc1688 (US_SG_FXMC trial) out of HK. All verified `left`, no stuck bans; 4 TELEGRAM_REMOVED rows in the Status Log (704–707). These were the runbook's expected-legitimate-difference class (drift scheduler.py ignores) — verified and explained, NOT a soak divergence. Next noon sweep ping should be silent.
...
> **✅ CUTOVER EXECUTED 2026-08-17 ~18:40 SGT** (Joseph's "move to production" call; done a day ahead of the planned Tue 18 Aug). Final soak check same evening covered the 16 Aug DrWealth conversion wave: webhook processed the whole wave cleanly (deferred welcomes fired on the 01:00 charges, downgrades confirmed, 4 payment-failure pings), bot.py convergent (wrote col P for the wave joiners it allowed), and the dry-run sweep's only candidate was @darrcyc (c16668@gmail.com, row CANCELLED, sole fully-barred row with a col-P ID) — legitimate. maxDuration verified empirically: the 13 Aug noon sweep ran 4m21s in a single invocation (Status Log rows 12:29:55→12:34:16), impossible under a 60s cap. Sheets 300/min quota bump status UNCONFIRMED (service account lacks quota-viewer; low risk now the wave has passed). Steps taken: 3 dry-run flags set `false` + fresh `TELEGRAM_WEBHOOK_SECRET` on prod via `printf | vercel env add`, prod redeployed (aliased, Ready), prod webhook registered (`getWebhookInfo` shows the prod URL + `allowed_updates: ["chat_member"]`, endpoint 403s without the secret), then the first LIVE sweep ran: `dryRun:false`, removed @darrcyc from HK_MARKET + MAIN_GROUP, zero failures/stuck bans. `.vercel/project.json` restored to staging afterwards. **Steps 3–5 completed by Joseph same evening (~19:05 SGT):** VPS bots stopped, verification join passed (@kxjiuhao joined US_MARKET and was kicked — correct not-found verdict; webhook logs show the chat_member updates answered 200), and @RhoNavStaginBot removed from the 5 real groups. **Confirm the VPS bots' auto-start is also disabled** (Task Scheduler / startup scripts) so a reboot can't resurrect them. Migration is now in the post-cutover watch: pings + Status Log for 48h, then 2–4 clean weeks before retirement (plan Task 12).
