...
> **PROGRESS (verified against git 2026-08-07):** Tasks 1–9 complete and pushed (Tasks 1–7 code commits `d32307e`…`2b5d994` + review-fix commits; Task 8 docs + Task 9 runbook in `3af8cad`). Task 10 complete — staging rig built and E2E matrix executed 2026-08-06, 11/11 pass, recorded in `docs/runbooks/telegram-migration-cutover.md` (Phase B banner). **Task 11 (prod shadow soak) IN PROGRESS — day 1 = 2026-08-07:** shadow bot @RhoNavStaginBot admin in the 5 real groups, staging repointed to real groups + live sheet (all flags dry-run), prod `TELEGRAM_CHAT_*` + whitelist configured + redeployed (they were missing — instant removal had been a silent Noop in prod), VPS bots updated + restarted, @RhoNavStaginBot whitelisted in `config.py` + both Vercel projects. Task 12 (cutover + retirement) not started. Staging cron noise (nightly test-bot failure pings from the shared `vercel.json` crons running without `STRIPE_*`/`RESEND_*`) was silenced 2026-08-07 by removing `CRON_SECRET` from the staging project — all staging crons now 401 quietly; the join webhook doesn't use it, and the sweep soak stream comes from the PROD sweep's dry-run ping.
