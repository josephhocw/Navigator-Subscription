...
The trial flow keys off Stripe's trial *mechanics*, so any trial the business runs gets it automatically. Cohort-specific content (the 25 July trial-group link, the 9 Aug date) lives only in the trial *welcome-at-signup* variant; the conversion and win-back emails are cohort-agnostic. Three moments:

1. **Sign up with a trial** → `STARTED` with `isTrial` (subscription arrived `trialing`). `sendOnboardingEmail` sends the trial variant: one trial-group button, signal groups + announcement channel **withheld** (learn-first), "First charge 9 August" framing. Paid checkouts get the normal welcome.
